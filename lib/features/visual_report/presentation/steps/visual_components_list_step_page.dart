...
class VisualComponentsListStepPage extends StatefulWidget {
  ...
  final Future<void> Function(VisualVictaulicGroupInspection)
  onVictaulicChanged;
...
class _VisualComponentsListStepPageState
  ...
  VisualComponentInspection? _editableFor(VisualComponentSequenceItem item) {
    ...
    return stored == null
        ? null
        : VisualComponentRules.suggestedDefaults(stored);
  }
  ...
  Widget _flowMeterCard() => Card(
    child: Padding(
      ...
      child: Column(
        ...
        children: [
          const Text(
            'Resumen canónico del paso 4',
            style: TextStyle(fontWeight: FontWeight.w900),
          ),
          const SizedBox(height: 8),
          Text(
            widget.flowMeterComplete
                ? 'La captura obligatoria del medidor está completa.'
                : 'Faltan datos obligatorios del medidor.',
          ),
          const Text(
            'No se crea una inspección duplicada para este componente.',
          ),
          const SizedBox(height: 8),
          OutlinedButton(
    ...
    if (definition.type == VisualComponentType.pressureGauge) {
      return Column(
        children: [
          ...
          const Text(
            'Lectura visual no certificada. No sustituye una medición del REPORTE FUNCIONAL.',
          ),
        ],
      );
    }
      ...
      return Column(
        children: [
          ...
          DropdownButtonFormField<String>(
            ...
            decoration: const InputDecoration(
              labelText: 'Visibilidad del conjunto',
            ),
            items: const [
              DropdownMenuItem(
                value: 'visible',
                child: Text('Visible exteriormente'),
              ),
              DropdownMenuItem(
                value: 'notVerifiable',
                child: Text('No verificable visualmente'),
              ),
            ],
            onChanged: widget.readOnly
                ? null
  ...
  void _updateJoint(
    ...
    setState(() {
      _victaulicDraft = group.copyWith(
        individualJoints: [
        ...
        if (condition != ObservedCondition.noVisibleDamage)
          CheckboxListTile(
            ...
            onChanged: widget.readOnly
                ? null
                : (selected) {
                    final next = {...value.observedConditions}
                      ..remove(ObservedCondition.noVisibleDamage);
                    if (selected == true) {
                      next.add(condition);
                    } else {
  ...
  String _buttonLabel(VisualComponentSequenceItem item) {
    if (widget.readOnly) {
      return _index == _sequence.length - 1 ? 'Ver resumen' : 'Siguiente';
    }
    if (item.kind == VisualSequenceItemKind.canonicalFlowMeter) {
  ...
  Future<void> _confirmAndContinue() async {
    ...
    if (item.kind == VisualSequenceItemKind.canonicalFlowMeter) {
      if (!widget.flowMeterComplete) {
        setState(
          () => _validationMessage =
              'Completa primero los datos obligatorios del paso 4.',
        );
        return;
      }
  ...
  void _advance() {
    setState(() {
      _index++;
      _draft = _index < _sequence.length
          ? _editableFor(_sequence[_index])
          : null;
      _victaulicDraft = null;
      _validationMessage = null;
    });
        ...
        child: ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            return ListTile(
              ...
              leading: Icon(
                confirmed ? Icons.check_circle : Icons.pending_outlined,
              ),
  ...
  Widget _summary(List<VisualComponentSequenceItem> sequence) {
    ...
    final findings = sequence
        .where((item) => _storedFor(item)?.hasFinding == true)
        .length;
    final photos = widget.components.fold<int>(
      0,
      (sum, item) => _inCurrentSection(item) ? sum + item.photoIds.length : sum,
    );
    final title = widget.section == VisualComponentsSection.publicNetwork
