...
abstract final class RvWorkDashboardProjection {
  ...
  static RvWorkGroup forDraft(RvDraft draft) {
    ...
    if (draft.isInactive) return RvWorkGroup.pendingSync;
    if (const {
      'returned',
      'rejected',
      'reopened',
  ...
  static Map<RvWorkGroup, Set<String>> byHydrant({
    ...
    for (final hydrant in hydrants) {
      ...
      final visible = local.toList(growable: false);
      final current =
  ...
  /// Lossless history projection used by "Todas mis revisiones". Unlike the
  /// compact dashboard it never deduplicates by hydrant or status.
  static List<RvDraft> allReviews(List<RvDraft> drafts) {
    final result = [...drafts]
      ..sort((a, b) => b.updatedAt.compareTo(a.updatedAt));
